...
| R2/C-4 | 终点里程兜底:g_s2_active 且自出箱Δ≥FINISH_FROM_S2_CNT(200cnt)→arm finish | main.c P1 前 | NAVSEG_FINISH_DIST_BACKUP(1) |
...
**下轮上图观察点(在 23:2X U1/U2 清单之上追加)**:sg= 应走 0→1→2→3→(4→5)→6;SM FORCED LATCH/EXIT 出现=兜底门工作(OLED);S② 区应 sm=1 且 T=14;跑完无 0x30 时车应在 S② 后 ~5m 内自停(FINISH DIST)。
---
