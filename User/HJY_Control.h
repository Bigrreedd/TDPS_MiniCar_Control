...
#define HJY_LAUNCH_HOLD_MS    1000u   /* HJY3(用户提议): K1 后静置窗——电机保持 0,
                                       * 去手扰/等风机起转;GO 瞬间才锁航向基准+
                                       * 开轮速窗+打前馈脉冲。HJY5: 1500→1000,
                                       * 用户报"按键~3s 才动"偏长(风机 kick 仅 200ms) */
...
#define HJY_ST_DEBOUNCE_TICKS 15u     /* HJY4c 丢线保持方向防抖:状态须连续(~30ms)
                                       * 才能成为"丢线记忆",杀横穿残影误改向。
                                       * HJY6: 25→15——阻尼后扫线 ~95°/s 每带驻留
                                       * ~45ms,须让正经横穿能更新记忆(过线后线在
                                       * 另一侧,记忆同步翻向才能收敛);单帧残影
                                       * (2~6ms)仍远不过阈 */
#define HJY_UNWIND_DROP       10.0f   /* HJY4b 快退绕触发阈:目标一次跌 >10cnt/s */
/* HJY5(17:20 第三跑): 爬行治好破粘不对称(双轮同帧 el=1 er=1)但曝出"破粘突奔"——
 * 静摩擦阈 1170/1290 ≫ 动摩擦需求,破粘瞬间 40+cnt/s 暴冲 30cm 右偏 20° 丢线;
 * 后段盲走 L3 档 300°/s 扫线必过头(yw 71→166 触安全网)。 */
#define HJY_CREEP_RAMP_PER_TICK 1.0f  /* HJY5a 爬行期死轮 tick 级附加爬坡(+500/s,
