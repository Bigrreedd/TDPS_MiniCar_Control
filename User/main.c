...
static uint16_t g_seg3_wide_run    = 0; /* F47: 宽黑横杠连续 tick(去抖:≥阈才计路口) */
...
/* F48(用户架构纠正"非路口巡线,只路口写死"+取证:拐角不呈宽黑只4个T/+可检测):
 * 巡线为默认;只在4个真 T/+ 横杠写死"分叉偏置"。两个90°拐角(750,50/750,200)=线弯,
 * 灰度黑少检测不到,交巡线自然过(车跟线弯=自然右转)。4横杠分叉表(按宽黑计数0..3索引):
 * (775,50)左 / (750,100)直 / (750,150)直 / (775,200)左。-1=左偏置 0=直穿(原生路口冻结) +1=右偏置。 */
static const int8_t g_seg3_branch[4] = {-1, 0, 0, -1};
static int8_t   g_seg3_bias_dir   = 0;    /* F48: 当前分叉偏置方向(-1左/+1右/0无);非0=偏置中 */
static float    g_seg3_bias_start  = 0.0f; /* F48: 偏置起始航向(rad),预算兜底基准 */
static float    g_seg3_bias_tgt    = 0.0f; /* F49: 偏置绝对目标航向(rad);到角(±tol)释放=IMU控角不过转 */
static uint8_t  g_seg3_jog_phase   = 0u;   /* F49: 底部左-右 jog 相(0无/1左到西/2右回北) */
...
#define SEG3_JCT_BLACK   6u      /* F48: black_count≥6(=SENSOR_COUNT-1)=垂直T/+横杠;90°拐角黑少(≤4)排除,
                                  * 兼杀离线自旋时的5黑误计(F47b Run2 spurious j3)。 */
#endif
#ifndef SEG3_BIAS_BUDGET_RAD
#define SEG3_BIAS_BUDGET_RAD 2.5f /* F49: 偏置 Δyaw 兜底上限(~143°,防到不了角卡死);主释放=到绝对目标角 */
#endif
#ifndef SEG3_JOG_WEST_RAD
#define SEG3_JOG_WEST_RAD 1.40f   /* F49: 底部 jog 相1 左转目标偏移≈80°(略欠90留过冲余量),相2回正北 */
#endif
#ifndef SEG3_BIAS_TOL
#define SEG3_BIAS_TOL 0.17f       /* F49: 到角容差≈10°(|add_angle-目标|≤此=该相完成) */
#endif
...
#endif
#ifndef SEG3_ACT_CPS
#define SEG3_ACT_CPS     18.0f   /* F47: 直穿动作 HEADING 爬行速(F46=12,去sm后floor70主导,设18有意义) */
#endif
#ifndef SEG3_HOLD_CENTER_BAND
#define SEG3_HOLD_CENTER_BAND 1.2f /* F46: |precise-3|≤此=在线居中,才刷新直穿航向锚 */
#endif
#ifndef SEG3_JCT_DEBOUNCE_TICKS
#define SEG3_JCT_DEBOUNCE_TICKS 5u /* F49: 8→5(检测②修):~25cps下横杠 bk≥6 窗口偏短,降去抖防漏检首横杠 */
#endif
#ifndef SEG3_TURN_BUDGET_RAD
#define SEG3_TURN_BUDGET_RAD 2.0f /* F47: 转弯 Δyaw 强制完成上限(~115°),全黑横杠下永不卡死 */
#endif
#ifndef SEG3_STRAIGHT_BUDGET_TICKS
#define SEG3_STRAIGHT_BUDGET_TICKS 400u /* F47: 直穿 tick 兜底(~0.8s)防黑数不落而卡死 */
#endif
#ifndef SEG3_CRUISE_CPS
#define SEG3_CRUISE_CPS  20.0f   /* F47: 丢线地基 HEADING 走直速(floor70 下实跑~20cps) */
#endif
#ifndef SEG3_TURN_CPS
#define SEG3_TURN_CPS    20.0f   /* F47: NAV_OVERRIDE_TURN 锐弧速(外轮=speed+差速,内轮coast) */
#endif
#define SEG3_TELEM_FMT  " s3w=%d j3=%d bk=%d bd=%d"   /* F42 s3w;F46 j3=过路口数,bk=黑数;F49 bd=偏置(-1左/+1右/0无) */
#define SEG3_TELEM_ARG  , (int)g_seg3_stop_run, (int)g_seg3_jct, (int)result_BlackPoint.black_count, (int)g_seg3_bias_dir
#else
...
static void SegTest_SeedOnStart(void)
{
    ...
    /* F47(用户三跑实证+25-agent取证): sm 种子已删 —— 方块区 ZIGZAG 不能用 S 配方常驻。
     * sm=1 把路口间(act==0)变成 F44 垃圾 pivot 温床:deep_enter 降1.5、A2锁向±320、内轮
     * min_inner=0 硬停 → 到路口前就 spin 飞(Run2/3 deep=1@j3=0)。去 sm 后 g_s_mode≡0:
     * deep_enter 回1.9(PID:886)、A2 锁向支路死(PID:649)、内轮回 min_inner=20(PID:927)。
     * 锚 g_s_mode_done=1 / g_sm_cnt_base=avg 由上方 >=3 块保留;转向/直穿由 F47 状态机写死执行。
    ...
    while (1)
    {
        ...
        if (g_control_tick)
        {
            ...
            if (is_racing)
            {
                ...
                {
                    /* F46/F47/F48: 出口门仅在末(第4)横杠分叉完成后武装(jct>=4 且 非偏置中)。基准=
                    ...
                    if (g_seg3_jct >= 4u && g_seg3_bias_dir == 0)
                    {
            ...
            else
            {
                ...
                g_seg3_act      = 0u;
                g_seg3_act_turn = 0u;
                g_seg3_wide_run    = 0u;  /* F47 */
                g_seg3_seen_center = 0u;
                g_seg3_straight_run= 0u;
                g_seg3_bias_dir    = 0;   /* F48: 分叉偏置停车态自清 */
                PID_SetBranchBias(0);
                g_seg3_jog_phase   = 0u;  /* F49 */
            }
#endif
#endif

#if TEST_SEGMENT == 3
            /* F48(替换F47b,用户架构纠正"非路口巡线只路口写死"+4-agent取证): 巡线为默认,只在
             * 4个真 T/+ 横杠写死分叉偏置。§3 纯坐标:黑线=下段(775,0→50)/上段(775,200→473)/三方块轮廓;
             * 入口(775,50)向北→出口(775,200)。用户坐标级路线: (775,50)左→西/(750,50)右→北/(750,100)直/
             * (750,150)直/(750,200)右→东/(775,200)左→北。其中 (750,50)/(750,200) 是90°拐角(线弯,
             * 灰度黑少 bk≤4 检测不到)→交巡线自然过(车跟线弯=自然右转);只有4个真 T/+ 横杠
             * (775,50/750,100/750,150/775,200)宽黑可检测,分叉表 g_seg3_branch[4]={左,直,直,左}。
             * F47b死因(4-agent取证): ①6事件表错(拐角不呈宽黑,j3卡1);②开环TURN甩车离线后不巡线
             * (用户"完全没巡线")。F48修: 默认巡线(质心环跟直线+90°拐角,同U弯);只在真横杠武装"分叉偏置"
             * (巡线域内 PID 注±130 中等corr+强制deep=0,两轮都驱动不甩离线),重捕干净单线/Δyaw预算即释放;
             * 直穿横杠走原生 is_junction 冻结;丢线交质心环重捕+375兜底(删了 F47 的盲航向保持漂)。
             * NAV_OVERRIDE_TURN 本区不再用(保留死码=比赛字节不变)。出口=末(第4)横杠偏置完成(jct≥4且
             * 非偏置中)记g_seg3_exitref,顶部左转触发上方IMU门。全程#if TEST_SEGMENT==3,比赛构型行为级不变。 */
            if (is_racing)
            {
                /* —— 每 tick 块顶:宽黑去抖 + 边沿复位 + 入口死区武装(与偏置状态解耦) —— */
                uint8_t seg3_bc   = result_BlackPoint.black_count;
                uint8_t seg3_wide = (seg3_bc >= SEG3_JCT_BLACK);   /* ≥6=真 T/+ 横杠(拐角≤4不计) */
                if (seg3_wide)
                {
                    if (g_seg3_wide_run < 0xFFFFu) g_seg3_wide_run++;
                }
                else
                {
                    g_seg3_wide_run = 0u;
                    g_seg3_inj = 0u;                 /* 真离杠才复位边沿,准备识别下一路口 */
                }
                if (result_BlackPoint.found && !seg3_wide)
                {   /* 见干净居中线→武装入口死区(此前不计路口,滤发车瞬态) */
                    float seg3_dp = result_BlackPoint.precise_position - 3.0f;
                    if (seg3_dp < 0.0f) seg3_dp = -seg3_dp;
                    if (seg3_dp <= SEG3_HOLD_CENTER_BAND) g_seg3_seen_center = 1u;
                }

                if (g_seg3_bias_dir != 0)
                {   /* —— 偏置中:留巡线域(NONE),PID 注固定偏置把车转到"绝对目标航向"(IMU控角=不过转,治F48过转) —— */
                    float seg3_err  = add_angle - g_seg3_bias_tgt;   if (seg3_err  < 0.0f) seg3_err  = -seg3_err;
                    float seg3_prog = add_angle - g_seg3_bias_start; if (seg3_prog < 0.0f) seg3_prog = -seg3_prog;
                    if (seg3_err <= SEG3_BIAS_TOL || seg3_prog >= SEG3_BIAS_BUDGET_RAD)
                    {
                        if (g_seg3_jog_phase == 1u)
                        {   /* 底部 jog:相1(到西)完成 → 相2 右偏回正北,落到 x=750 线 */
                            g_seg3_jog_phase  = 2u;
                            g_seg3_bias_dir   = 1;             /* 右偏(yaw 减) */
                            g_seg3_bias_start = add_angle;
                            g_seg3_bias_tgt   = g_yaw_zero;   /* 目标=正北 */
                            PID_SetBranchBias(1);
                        }
                        else
                        {   /* 单偏置 或 jog 相2 完成 → 释放,交回纯巡线 */
                            PID_SetBranchBias(0);
                            g_seg3_bias_dir  = 0;
                            g_seg3_jog_phase = 0u;
                            if (g_seg3_jct >= 4u) g_seg3_exitref = add_angle;  /* 末(第4)横杠完成→立出口基准 */
                        }
                    }
                    PID_SetNavOverride(NAV_OVERRIDE_NONE, 0.0f, 0.0f);        /* 偏置=巡线域,非 override */
                }
                else if (g_seg3_wide_run >= SEG3_JCT_DEBOUNCE_TICKS && !g_seg3_inj &&
                         g_seg3_seen_center && g_seg3_jct < 4u)
                {   /* —— 确认真 T/+ 横杠:按位置武装写死偏置(转到绝对目标航向) —— */
                    uint8_t seg3_i = (uint8_t)g_seg3_jct;
                    g_seg3_inj = 1u;
                    g_seg3_jct++;
                    if (seg3_i == 0u)
                    {   /* 底部(775,50):左-右 jog。相1 左偏到西(yaw_zero+80°);相2(右回北)由上方完成时接力 */
                        g_seg3_jog_phase  = 1u;
                        g_seg3_bias_dir   = -1;               /* 左偏(yaw 增) */
                        g_seg3_bias_start = add_angle;
                        g_seg3_bias_tgt   = g_yaw_zero + SEG3_JOG_WEST_RAD;
                        PID_SetBranchBias(-1);
                    }
                    else if (seg3_i == 3u)
                    {   /* 顶部(775,200)T:车经(750,200)拐角后朝东,左偏回正北(yaw_zero) */
                        g_seg3_jog_phase  = 0u;
                        g_seg3_bias_dir   = -1;               /* 左偏(yaw 增,东→北) */
                        g_seg3_bias_start = add_angle;
                        g_seg3_bias_tgt   = g_yaw_zero;
                        PID_SetBranchBias(-1);
                    }
                    /* 横杠1,2(750,100/150)=直穿:不武装偏置,原生 is_junction 冻结走直过杆。
                     * (750,50)拐角=底部 jog 相2 完成;(750,200)拐角=巡线跟弯,均不在此处写死。 */
                    PID_SetNavOverride(NAV_OVERRIDE_NONE, 0.0f, 0.0f);
                }
                else
                {   /* —— 默认:纯巡线(NONE)。直线/(750,200)拐角/丢线重捕交质心环(同 U 弯) —— */
                    PID_SetNavOverride(NAV_OVERRIDE_NONE, 0.0f, 0.0f);
                }
            }
#endif
        ...
        if (event != NULL)
        {
            ...
            case KEY_K1:
                ...
                g_seg3_bias_dir    = 0;    /* F48: 分叉偏置复位 */
                PID_SetBranchBias(0);
                g_seg3_jog_phase   = 0u;   /* F49 */
                g_seg3_bias_tgt    = add_angle;
#endif
