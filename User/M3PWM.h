 ...
 * G3/G3b(06-07 用户拍板): 50→100→120,板级永久堵转仍全额定内(19:00 网表实证三颗
 * SS54FSH 并联,降额 ~10A: 堵转@12% 二极管 84·0.12·0.88=8.9A 总=2.96A/颗 ✓(总裕 11%)
 * XT30 10.1A<15A ✓ NMOS 无压力),烧不了板,唯一牺牲件=风扇电机自身(卡死本就报废)。
 * **120=绝对安全天花板**:130 起并联预算零裕度、150 超预算(10.7A>10A,故 kick 只许
 * 200ms 瞬态)——再上必须换 ≥20A 二极管或加电流采样,本参数就此锁死。
 * 台架 60s 摸温过测后与 FAN_RACE_HOLD_DUTY=120 一并定版;不过测双双回 50。
 * 电流采样仍缺,程序性替代=每轮 kick 可闻起转自检+人跟车 K2。 */
#define FAN_DUTY_ABS_CAP       120
